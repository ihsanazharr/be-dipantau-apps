...
// Helper: Check if user is himpunan admin
const isHimpunanAdmin = async (userId, himpunanId) => {
...
exports.createActivity = async (req, res) => {
  try {
    ...
    if (!title || !startDateTime || !endDateTime || !himpunanId) {
      return res.status(400).json({
        success: false,
        message: 'Missing required fields'
      });
    }
    // Verify himpunan exists
    ...
    // Authorization check
    const allowCreate = req.user.role === 'super_admin' ||
      await isHimpunanAdmin(req.user.id, himpunanId);
    ...
    // Generate QR code
    const qrPayload = `act:${crypto.randomBytes(16).toString('hex')}`;
